...
const Tasks = ({ telegramUser, botUsername }) => {
  ...
  useEffect(() => {
    ...
    const handleRankStats = (data) => {
      if (data.success) setGameStats(data);
    };
  ...
  const claimTaskReward = useCallback(
    (taskId, additionalData = {}) => {
      if (!socket || !telegramUser) return;
      socket.emit(
        "tasks:claimReward",
        { telegramId: telegramUser.id, taskId, ...additionalData },
        (res) => {
          if (res?.success) {
            loadTasks();
            loadReferralStats();
            ...
            showNotification(
              res.message ||
                `Task reward claimed! +${res.rewardAmount || 0} Gold Pieces`
            );
            ...
            if (res?.requiresVerification) {
              showNotification(
                'Please join the channel first, then click "Claim"',
                "error"
              );
            } else if (res?.requiresConfirmation) {
              showNotification(
                'Please follow the account first, then click "Claim"',
                "error"
              );
            } else {
    ...
    if (task.channelLink) {
      ...
      if (window.Telegram?.WebApp) {
        try {
          ...
          showNotification(
            'Opening channel... Join the channel and return here to click "Claim"!',
            "success"
          );
          ...
          showNotification(
            'Channel link opened! Join the channel and return here to click "Claim"!',
            "success"
          );
          // Change button state to 'claim'
          setButtonStates((prev) => ({ ...prev, join_channel: "claim" }));
      ...
      (res) => {
        if (res?.success) {
          if (res.isMember) {
            ...
            showNotification(
              "Channel membership verified! Claiming your reward...",
              "success"
            );
            ...
            const confirmed = window.confirm(
              "Please confirm that you have joined the channel. Click OK to claim your reward."
            );
            if (confirmed) {
              showNotification(
                "Manual verification confirmed! Claiming your reward...",
                "success"
              );
              claimTaskReward(task.id);
              // Reset button state after successful claim
            ...
            showNotification(
              res.message || "Please join the channel first.",
              "error"
            );
          }
        } else {
          ...
          if (res?.error?.includes("member list is inaccessible")) {
            showNotification(
              "Bot verification failed. Using manual verification instead...",
              "warning"
            );
            ...
            showNotification(
              res?.error || "Failed to verify channel membership",
              "error"
            );
          }
        }
  ...
  const handleXFollow = (task) => {
    if (task.xLink) {
      ...
      showNotification(
        'Opening X profile... Follow and return here to click "Claim"!',
        "success"
      );
      // Change button state to 'claim'
  ...
  const handleYouTubeSubscribe = (task) => {
    if (task.youtubeLink) {
      ...
      showNotification(
        'Opening YouTube channel... Subscribe to and return here to click "Claim"!',
        "success"
      );
      // Change button state to 'claim'
  ...
  const handleInstagramFollow = (task) => {
    if (task.instagramLink) {
      ...
      showNotification(
        'Opening Instagram profile... Follow and return here to click "Claim"!',
        "success"
      );
      // Change button state to 'claim'
  ...
  const handleUseReferralCode = () => {
    ...
    socket.emit(
      "referral:useCode",
      { telegramId: telegramUser.id, referralCode: referralInput.trim() },
      (res) => {
        if (res?.success) {
  ...
  const getTaskProgress = (task) => {
    ...
    switch (task.id) {
      case "play_games":
        progress = Math.min(gameStats.totalGamesPlayed || 0, task.target);
        break;
      case "win_games":
        progress = Math.min(gameStats.totalGamesWon || 0, task.target);
        break;
      case "kill_pawns":
        progress = Math.min(gameStats.totalPawnsKilled || 0, task.target);
        break;
      case "first_home":
        progress = Math.min(gameStats.totalFirstHome || 0, task.target);
        break;
      case "referral":
      ...
      case "daily_login":
        ...
        const lastDaily = task.lastDailyReward
          ? new Date(task.lastDailyReward).toDateString()
          : null;
  ...
  const renderTasks = () => {
    if (tasksLoading || !gameStats)
      return (
        <div className="flex justify-center items-center py-4">
          <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-cyan-400"></div>
          {!gameStats && (
            <div className="ml-2 text-gray-400 text-xs">Loading...</div>
          )}
        </div>
      );
    if (error)
                ...
                <div className="flex items-center gap-4 flex-1">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-sm text-gray-400 font-medium">
                      +{getTaskEnergyReward(task).toLocaleString()} Energy
                    </div>
                    {task.target > 1 && (
                      <div className="text-xs text-gray-400 mt-2">
                        <span>
                          {getTaskProgress(task)}/{task.target}
                        </span>
                        <div className="w-full bg-gray-700 rounded-full h-1.5 mt-1">
                          <div
                            className="bg-gradient-to-r from-gray-500 to-gray-500 h-1.5 rounded-full transition-all duration-300"
                            style={{
                              width: `${Math.min(
                                (getTaskProgress(task) / task.target) * 100,
                                100
                              )}%`,
                            }}
                          ></div>
  ...
  const renderReferralSection = () => {
    ...
    return (
      <div className="space-y-3">
        <motion.div
          className="rounded-lg p-3 border border-gray-700/40"
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
        >
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 pb-24">
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                ...
                <p className="text-[11px] text-gray-400 mb-3 leading-relaxed">
                  Complete tasks, earn rewards, and get bonuses for active
                  friends!
                </p>
                <motion.button
                  onClick={() => {
        ...
        >
          <div className="max-w-xl mx-auto px-4">{renderTabContent()}</div>
        </motion.div>
      </div>

      {notification.show && (
        <motion.div
          className={`fixed right-4 p-3 rounded-lg shadow-lg z-50 text-sm ${
