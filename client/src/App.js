...
function AppContent() {
  ...
  useEffect(() => {
    const newSocket = io(process.env.REACT_APP_GAME_URL, {
      path: '/api/socket.io',
      ...
      <div>
        ...
        <div className="fixed top-4 right-4 z-50 space-y-2">
          {notifications.map((notification) => (
            <div
              key={notification.id}
              className={`px-4 py-3 rounded-lg shadow-lg text-white max-w-sm transform transition-all duration-300 ${notification.type === 'success' ? 'bg-green-500' :
...
function App() {
  ...
  const getManifestUrl = () => {
    ...
    return `${process.env.REACT_APP_GAME_URL}/tonconnect-manifest.json`;
  };
